...
// Static Data for Charts (Simulated for now)
const evolutionData = [
  { name: 'Jan', value: 30 },
  { name: 'Fev', value: 45 },
  { name: 'Mar', value: 120 },
...
const Dashboard: React.FC = () => {
  const { stats, neighborhoods, birthdays, loading } = useDashboard();
  const [viewMode, setViewMode] = useState<'list' | 'map'>('list');
  const [selectedBirthday, setSelectedBirthday] = useState<any>(null);

  if (loading) {
    return <div className="p-8 text-center text-slate-500">Carregando painel...</div>;
  }
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
          <div className="bg-white/5 border border-white/10 rounded-xl p-4 hover:bg-white/10 transition-colors cursor-pointer">
            ...
            <p className="text-sm font-medium">{stats.activeDemands > 0 ? `${stats.activeDemands} demandas ativas exigem atenção.` : 'Nenhuma demanda crítica no momento.'}</p>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-4 hover:bg-white/10 transition-colors cursor-pointer">
            ...
            <p className="text-sm font-medium">{birthdays.length > 0 ? `${birthdays.length} aniversariantes esta semana.` : 'Nenhum aniversariante próximo.'}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-bold text-lg text-slate-800 dark:text-white">Crescimento da Base (Simulado)</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-6">
          ...
          {viewMode === 'list' ? (
            <div className="space-y-3">
              {neighborhoods.length === 0 ? <p className="text-slate-500 text-sm">Nenhum dado de bairro.</p> :
                  ...
                  <div key={idx} className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-slate-700 last:border-0 hover:bg-gray-50 dark:hover:bg-slate-700/50 px-2 rounded-lg transition-colors cursor-pointer group">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-24 h-1.5 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                        <div className="h-full bg-primary-600 rounded-full" style={{ width: `${(bairro.count / (stats.totalVoters || 1)) * 100}%` }}></div>
                      </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-6">
          ...
          <div className="space-y-1">
            {birthdays.length === 0 ? <p className="text-slate-500 text-sm">Nenhum aniversariante próximo.</p> :
                ...
                >
                  <div className="flex-1 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400">{person.neighborhood || person.city}</p>
                    </div>
                  </div>
